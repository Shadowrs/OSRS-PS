 ...
 * @author Emperor
 */
public final class DwarfCannonHandler {
	 ...
	 */
	private DwarfCannonRevolution direction = DwarfCannonRevolution.NORTH;
	...
	/**
	 * Constructs a new {@code DwarfCannonHandler} {@code Object}
	 * @param player The player owning the cannon.
	 */
	public DwarfCannonHandler(final Player player) {
		this.player = player;
	...
	private boolean rotate() {
		...
		direction = DwarfCannonRevolution.values()[(direction.ordinal() + 1) % DwarfCannonRevolution.values().length];
	...
	public static void construct(final Player player) {
		...
		final DwarfCannonHandler handler = new DwarfCannonHandler(player);
